...
class GaugeFrame(customtkinter.CTkFrame):
    def __init__(self, master, network:canopen.Network=None, db:cantools.db.Database=None):
        ...
        self.network.subscribe(can_id=message.frame_id, callback=lambda x,y,z,s=signal,g=self.gauge1:self.gauge_callback(x,y,z,s,g))
        
        # Register Callback for the Current Gauge
        signal = message.get_signal_by_name('Current')
        self.network.subscribe(can_id=message.frame_id, callback=lambda x,y,z,s=signal,g=self.gauge2:self.gauge_callback(x,y,z,s,g))
        
        # Register Callback for the SoC Gauge
        signal = message.get_signal_by_name('StateOfCharge')
        self.network.subscribe(can_id=message.frame_id, callback=lambda x,y,z,s=signal,g=self.gauge3:self.gauge_callback(x,y,z,s,g))
    ...
    def gauge_callback(self, can_id, data, timestamp, signal:cantools.db.Signal, gauge):
