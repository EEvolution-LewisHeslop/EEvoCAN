...
class DbcFrame(customtkinter.CTkFrame):
    ...
    def load_dbc(self, filePath=None):
        # TODO: handle clearing of active dbc callbacks if a dbc is already in use
        if (filePath is None):
    ...
    # When called, updates the values column at a given row based on the value received for a given signal.
    def dbc_callback(self, can_id, data, timestamp, rowcount, signal:cantools.db.Signal):        
