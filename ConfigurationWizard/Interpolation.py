...
class Interpolation:
    # Takes an 1 dimensional list of numbers and '' elements
    # and replaces '' elements with interpolations to next value.
    # Requires that at least the first
    # and last elements of the list are populated.
    def LinearFill(listToFill):
        ...
        for i in range(0, len(listToFill)):
            if (str(listToFill[i]).strip() != ''):
                ...
                nullCount += 1
        if (nullCount == 0):
            print("Unable to interpolate because no \"null\" "
                  "elements in given list.")
            return listToFill
        if (numberCount <= 2):
            # Only one operation is required
            # and the whole range is a valid start.
            ...
            for i in range(0, len(listToFill)):
                if (str(listToFill[i]).strip() == '' and not runStarted):
                    ...
                    runStarted = False
                    runEndAt = i
                    newList.extend(Interpolation.LinearInterpolate(
                        listToFill[runStartAt:runEndAt+1:],
                        runCount)[1::])
                    runCount = 0
                elif (runStarted):
                    runCount += 1
            ...
            # All runs done now piece them together
            # with the beginning and the ends.
            return newList

    # Takes a list with '' elements and numbers at either end.
    # Interpolates between the two numbers and replaces '' elements.
    # Sanitise all data beforehand.
    def LinearInterpolate(listToInterpolate, nullCount):
        ...
        # Assume last element is greater.
        lastFloat = float(listToInterpolate[len(listToInterpolate) - 1])
        firstFloat = float(listToInterpolate[0])
        difference = float(lastFloat - firstFloat)
        # Calculate the step.
        step = difference / (nullCount + 1)
        # Fill nulls.
        for i in range(0, nullCount):
            newVal = float(listToInterpolate[0]) + ((i + 1)*step)
            listToInterpolate[i + 1] = newVal
            print(f"Filling null {i+1} with value {listToInterpolate[i + 1]}.")
        # Return the updated list.
        return listToInterpolate

    # Takes a 2 dimensional array of points with at least 4 values
    # and fills in the missing values by 3rd order interpolation
        ...
        for row in tableOfValues:
            ...
            for element in row:
                ...
                if (str(element).strip() == ''):
                    ...
                    newRow.append(ndimage.map_coordinates(
                        npArray.T,
                        [[xCoord], [yCoord]],
                        order=1)[0])
                else:
    ...
    # Takes a 2 dimensional array of points with at least 4 values
    # and fills in the missing values by Clough Tocher 2D interpolation
    def ScatteredBicubicInterpolation(tableOfValues, colMax, rowMax):
        ...
        # Check to see if there is enough information
        # to construct initial simplex.
