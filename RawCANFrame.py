...
class RawCANFrame(customtkinter.CTkFrame):
    ...
    def __init__(self, master, network=None):
        ...
        self.scrollToBottomCheckbox = customtkinter.CTkCheckBox(self, text="Scroll to Bottom")
        self.scrollToBottomCheckbox.select()
        self.scrollToBottomCheckbox.grid(row=0, column=0, padx=20, pady=10, sticky='e')
    ...
    def assign_listener(self):
        # Create a listener
        listener = RawCANListener(self.canText, self.led0, self.scrollToBottomCheckbox)
        self.network.notifier.add_listener(listener)        
        print("Assigned listener to RawCANFrame")

class RawCANListener(Listener):
    def __init__(self, text:customtkinter.CTkTextbox, led:tk_tools.Led, scrollToBottom:customtkinter.CTkCheckBox):
        self.text=text
        self.led=led
